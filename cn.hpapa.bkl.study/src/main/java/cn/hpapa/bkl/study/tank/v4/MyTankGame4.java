 ...
 * 4、实现子弹的连发(最多连发5颗子弹)
 */
package cn.hpapa.bkl.study.tank.v4;
...
import cn.hpapa.bkl.study.tank.Shot;
...
//		g.drawArc(20, 20, 100, 200, 50, -100);
		...
		for(int i = 0; i < hero.getvShots().size(); i++){
			Shot myShot = hero.getvShots().get(i);
			// 画一颗子弹
			if(myShot != null && myShot.isLive()) {
				g.drawRect(myShot.getX(), myShot.getY(), 1, 1);
			}
		}
